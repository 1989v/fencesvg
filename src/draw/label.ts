 ...
 * 전부 이 모양(가운데 정렬 칩)을 쓴다 — 한 곳에 모아 반경·패딩·불투명도가
 * 다섯 군데에서 어긋나지 않게 한다. class 의 관계 라벨은 세로선 옆에 붙는
 * 왼쪽 정렬 칩이라 띄우는 축이 달라(가로 gap) `anchor: 'start'` 로 받는다.
 */
export function edgeLabel(
  ...
  const body = [
    ...
    text(label, {
      x, y: baseline, 'text-anchor': anchor, fill: 'currentColor',
      'fill-opacity': opacity, 'font-size': fontSize, 'font-weight': WEIGHT.edgeLabel,
    }),
  ];
  return { body, box };
}
